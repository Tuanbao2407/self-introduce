...
export default function AudioPlayer({ src: initialSrc } = {}) {
  ...
  const loadExternal = () => {
    ...
    if (audioRef.current) {
      try {
        ...
        setError(
          "Failed to load the provided URL as audio. It may not be a direct media file.",
        );
      }
    }
  };
  ...
  const handleAudioError = () => {
    setError(
      "Unable to play the audio. The URL might not point to a direct audio file or CORS blocks playback.",
    );
  };
    ...
    <div className="fixed bottom-5 right-5 z-50 bg-black/80 backdrop-blur-sm p-4 rounded-lg border border-marshallGold/30 shadow-lg">
      ...
      <div className="flex items-center space-x-4">
        ...
        <div className="flex items-center space-x-2">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            className="h-5 w-5 text-marshallGold"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
            <path
              ...
              strokeLinejoin="round"
              strokeWidth={2}
              d="M15.536 8.464a5 5 0 010 7.072m2.828-9.9a9 9 0 010 12.728M5.586 15.536a5 5 0 001.414-3.536 5 5 0 00-1.414-3.536M2.757 17.657a9 9 0 012.829-6.364 9 9 0 012.828-2.828"
            />
          </svg>
          <input
            type="range"
